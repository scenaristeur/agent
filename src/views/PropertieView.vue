  ...
  <div>
    ...
    <div v-if="typeof v == 'object'">
      ...
      <span v-if="Array.isArray(v)">
        ...
        <span v-for="(val, i) in v" :key="i">
          <b-button
            v-if="val.id != undefined"
            size="sm"
            @click="switchTo(val.id)"
            variant="outline-primary"
            >{{ val.name || val.id }}</b-button
          >
          <span v-else>
            <b-badge variant="light">{{ val }}</b-badge></span
          >
        </span>
    ...
    <div v-else>
      <div v-if="p == 'base64'">
        ...
        <video
          v-else-if="v.startsWith('data:video')"
          controls
          max-width="100%"
          max-height="500px"
        >
