...
export default {
  ...
  created() {
    let coreStore = this.$store.state.core;
    this.commands.ls = ({ _ }) => {
      let nodes = coreStore.nodes;
      ...
      if (command.length === 1) {
        ...
        if (this.stdin !== "" && candidates.length > 1) {
          this.history.push({
            ...
            render: (createElement) => {
              ...
              const rows =
                candidates.length < 5 ? 1 : Math.ceil(candidates.length / columns);
              let index = 0;
              const table = [];
        ...
        if (candidates.length === 1) {
          ...
          const unwatch = this.$watch(
            ...
            () => {
              this.cursor =
                cursor + (candidates[0].length - autocompleteableStdin.length + 0);
              unwatch();
            }
          );
      ...
      if (autocompleteableStdin.substring(0, 2) === "--") {
        ...
        if (candidates.length === 1) {
          ...
          const unwatch = this.$watch(
            ...
            () => {
              this.cursor =
                cursor + (candidates[0].length - autocompleteableStdin.length + 2);
              unwatch();
            }
          );
      ...
      if (autocompleteableStdin.substring(0, 1) === "-") {
        ...
        if (candidates.length === 1) {
          ...
          const unwatch = this.$watch(
            ...
            () => {
              this.cursor =
                cursor + (candidates[0].length - autocompleteableStdin.length + 1);
              unwatch();
            }
          );
          this.stdin = `${autocompleted}${rest}`;
