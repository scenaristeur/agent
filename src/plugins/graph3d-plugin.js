...
const plugin = {
  ...
  // }
    ...
    Vue.prototype.$graphInit = async function(options){
      ...
      //   console.log("update", sprite, start, end)
    ...
    //       store.state.core.nodes.forEach((d, i) => {
    //         d.fx = (1 - dd.lambda) * objects[i].x + dd.lambda * targets[i].x;
    //         d.fy = (1 - dd.lambda) * objects[i].y + dd.lambda * targets[i].y;
    //         d.fz = (1 - dd.lambda) * objects[i].z + dd.lambda * targets[i].z;
    //         objects[i].fx = (1 - dd.lambda) * objects[i].x + dd.lambda * targets[i].x;
    //         objects[i].fy = (1 - dd.lambda) * objects[i].y + dd.lambda * targets[i].y;
    //         objects[i].fz = (1 - dd.lambda) * objects[i].z + dd.lambda * targets[i].z;
    //         console.log(d)
    //       })
    //     }
    //   });
...
//     function render(object, target){
//       console.log("object, target",object, target, object.position, target.position)
//       let name = target.name.split('_')[1]
//       var index = store.state.nodes.findIndex(x => x.name==name);
// console.log(index, store.state.nodes[index])
//       // index === -1 ? store.state.nodes.push(n) : Object.assign(context.state.nodes[index], n)
// // let node =store.state.core.graph.graphData()['nodes'].filter(x=> x.name== name)[0]
// // console.log(name, node)
// // node.x = target.position.x
// // node.y = target.position.y
// // node.z = target.position.z
// //
// //       store.dispatch('core/saveNode', node)
// // store.state.core.graph.refresh()
//
//     }
...
onNodeClick(e);
              ...
              // objectCSS.addEventListener('pointerdown', (e) => {
              //   //  alert("click element")
              //   console.log("pointerdown",objectCSS,e)
              // })
              //
              // objectCSS.addEventListener('click', (e) => {
              //   //  alert("click element")
              //   console.log("click", objectCSS,e)
              // })
